...
% overpotential
U0  = @(cs) (1-cs-Om2).*Om1 + (cs).*Om3;
eta = @(cs,pe,ps) ps-pe-U0(cs);
...
i0 = @(ce,cs) K*sq((cs.*(cmax*(1-Om2)-cs).*ce));
j  = @(ce,cs,pe,ps,ps2) A*(Om1+Om3).*i0(ce,cs)...
    .*sinj(alpha*F*eta(cs,pe,ps.*Om1(x)+ps2.*Om3(x)));
%jx = @(x,ce,cs,pe,ps) feval(j(pe,ps,ce,cs),x);
%jx = @(x,ce,cs,pe,ps) 1; % EXPLICIT
jx = @(ce,cs,pe,ps,ps2) A*(Om1+Om3)*...
    K.*((cs.*(cmax*(1-Om2)-cs).*ce)).* ...
    sinh(alpha*F*(ps.*Om1+ps2.*Om3-pe-((1-cs-Om2).*Om1 + (cs).*Om3))); %IMPLICIT
...
N = chebop(@(x,ce,cs,pe,ps,ps2) [De*diff(ce,2) + (1-t0)*jx(ce,cs,pe,ps,ps2)/F/n - ce/dt;...
    -jx(ce,cs,pe,ps,ps2)/F/n + Ds*diff(cs,2) - cs/dt; ...
    -k*diff(pe,2) - kD*diff(diff(ce)./ce) - jx(ce,cs,pe,ps,ps2); ...
    -sigma*diff(ps,2) + jx(ce,cs,pe,ps,ps2);...
    -sigma*diff(ps2,2) + jx(ce,cs,pe,ps,ps2)], dom);
...
for i=1:nt
    ...
    %jj=0; % IMPLICIT
    jj=(j(ce,cs,pe,ps,ps2)-jx(ce,cs,pe,ps,ps2))*dt; % EXPLICIT
%     figure(1)
%     hold on
